...
public class LgDispatcherServlet extends HttpServlet {
    ...
    private void initSecurity() {
        for (Handler handler : handlerMapping){
            ...
            String url = handler.getPattern().toString();
            Method method = handler.getMethod();
            //加在类上
            boolean hasSecurity = o.getClass().isAnnotationPresent(Security.class);
            if (hasSecurity) {
                String[] names = o.getClass().getAnnotation(Security.class).value();
                security.put(url,names);
                continue;
            }
            //加在方法上
            boolean hasSec = method.isAnnotationPresent(Security.class);
            if (hasSec) {
...
//        method.invoke() //
        ...
        //获取权限
        String[] names = security.get(req.getRequestURI());
        boolean flag = false;
        for (String name : names) {
            if (name.equals(req.getParameter("username"))) {
                flag = true;
                break;
            }
        }
